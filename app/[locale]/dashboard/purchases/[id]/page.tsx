...
const PurchaseOrderForm = () => {
  ...
  const fetchPurchaseOrder = useCallback(async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`/api/purchase/${routeParam?.id}`);
  ...
  const handleOrderNumberChange = useCallback((event: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleReset = useCallback(() => {
    ...
    setPurchaseOrder("");
    setFile("");
    setItems([{ inventory_id: "", quantity: 0, price: 0 }]);
  }, []);

  const handleItemChange = useCallback(
    (index: number, field: keyof PurchaseItem, value: any): void => {
  ...
  const dictionary = useCallback(async () => {
    try {
  ...
  const handleSubmit = useCallback(
    async (event: any) => {
      ...
      try {
        ...
        await axios.put(`/api/purchase/${routeParam?.id}`, formData);
    ...
    [file, items, vendor, order_number, routeParam?.id, routeParam?.locale, router]
  );
  ...
  const renderItems = useMemo(
    () =>
      items.map((item, index) => (
        <div key={index}>
          ...
          <div className="w-full flex align-center border border-neutral-300 rounded-sm p-2">
            <div className="border-r-[1px] gap-2 grid grid-cols-1 border-gray-200 p-4 w-full">
              <div className="p-o w-full">
                <label className="text-sm font-semibold" htmlFor="purchase-order">
                  {dict?.item || "Item"}<span className="ml-2 text-red-600">*</span>
                </label>
                <Select
                  onValueChange={(value) => handleItemChange(index, "inventory_id", value)}
              ...
              <div className="files w-full">
                <label className="text-sm font-semibold items-center" htmlFor="quantity">
              ...
              <div className="files w-full">
                <label className="text-sm font-semibold items-center" htmlFor="initial_price">
    ...
    [items, dict, loading, inventoryItems, handleItemChange, handleRemoveInventoryItem, router]
  );
      ...
      <div className="bg-white rounded-md w-full p-4 dark:bg-black">
        <div className="mb-4">
          <p className="text-md font-semibold ">
            {dict?.purchaseFormSubheading || " Fill in the form to register a purchase."}
          </p>
        </div>
        <form onSubmit={handleSubmit}>
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-4 w-full">
              ...
              <div className="files w-full">
                <label className="text-sm font-semibold items-center" htmlFor="files">
            ...
            {renderItems}
            <div
              className="border border-[#1C40CA] rounded-md font-semibold text-[#1C40CA] px-8 py-2 text-center cursor-pointer"
